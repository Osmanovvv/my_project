...
type Props = {
  /**
   * Тексты страницы: первый экран, «кому подходит», «что получите», шаги.
   * Приходят из базы через загрузчик роута — они правятся из админки.
   *
   * `null` возможен при сбое загрузчика. Тогда рисуется скелет страницы
   * без текстовых блоков, а не пустой экран с ошибкой: цена, срок и шапка
   * останутся на месте.
   */
  content: ServicePageContent | null;
  /** Значок направления рядом с плашкой. Оформление, не контент. */
  icon: LucideIcon;
...
export function ServiceVariationPage({
  ...
  return (
    <ServiceDetailLayout
      ...
      title={<AccentText text={content?.heroTitle ?? ""} />}
    ...
    >
      ...
      <section className="container-page pt-14">
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          {/* Третья плитка появляется, только когда есть что показать: пустая
              колонка рядом со сроком и ценой читается как поломка вёрстки. */}
          {bestFor.length > 0 && (
            <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      {features.length > 0 && (
        <section className="container-page py-16 sm:py-24">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map(({ icon: iconKey, title: t, text }) => (
              <div
                key={t}
              ...
              >
                <div className="grid h-11 w-11 place-items-center rounded-xl bg-accent-soft text-accent mb-4">
                  {/* Иконка приходит из базы ключом, а не компонентом: классы
                      и модули из базы сборщик не увидит. См. `Icon.tsx`. */}
                  <Icon name={iconKey} className="h-5 w-5" strokeWidth={1.75} />
                </div>
      ...
      {steps.length > 0 && (
        <section className="container-page py-16 sm:py-24 border-t border-border">
          ...
          {/* Без этой строки шаги противоречат общему сроку: сложив их, клиент
              ...
              Строка намеренно НЕ вынесена в админку: это страховка от спора
              «вы обещали три недели», и сокращённая на бегу до «сроки
              примерные» она теряет весь смысл. */}
      ...
      <section className="container-page pb-16 sm:pb-24">
        <div className="rounded-3xl border border-border bg-gradient-to-br from-accent-soft/60 via-background to-background p-8 sm:p-10">
          <div className="grid md:grid-cols-[1fr_auto] gap-6 items-center">
            <div>
              <h3 className="font-display text-2xl sm:text-3xl">{texts["service.cta.title"]}</h3>
              <p className="mt-2 text-muted-foreground max-w-xl">{texts["service.cta.lead"]}</p>
            </div>
            <Link
              to="/contacts"
